...
try:
    ...
    from deepnet.process.data_augmentation import cupyx_ndimage as cu_ndimage
except:
    pass


def rotate(input, angle, axes=(1, 0), reshape=True, output=None, order=0, mode='constant', cval=0.0, prefilter=True):
...
def shift(input, shift, output=None, order=0, mode='constant', cval=0.0, prefilter=True):
    xp = cuda.get_array_module(input)
    if xp == np:
...
def zoom(input, zoom, output=None, order=0, mode='constant', cval=0.0, prefilter=True):
    xp = cuda.get_array_module(input)
    if xp == np:
...
def affine_transform(input, matrix, offset=0.0, output_shape=None, output=None, order=0, mode='constant', cval=0.0, prefilter=True):
    xp = cuda.get_array_module(input)
    if xp == np:
