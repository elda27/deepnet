...
def build_networks(config, step=None):
    ...
    config = copy.deepcopy(
        config)  # not to affect changes in the other function.
    ...
    for network_conf in config['network']:
        ...
        if process_names[0] in _created_process:  # If process is exist.
            ...
            # registered_proc contains 'proc' and 'proerty', and so on.
            registered_proc = _created_process[process_names[0]]
    ...
    for network_conf in config['visualize']:
        ...
        visualizers.append(visualizer.create_visualizer(
            type_name)(**network_conf))
