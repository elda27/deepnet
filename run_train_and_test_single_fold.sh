...
if [ $# -lt 2 ]; then 
  echo "$0 <encode dims> <log index> <fold index> <stage index>" >&2
  exit -1
fi
...
echo "Stage ${STAGE_INDEX}-------------------------------------"
cmd="${TRAIN_CMD} 
  --gpu ${GPU_ID} 
  --train-index ${K_FOLD_CONFIG_ROOT}/${N_FOLD}/id-list_trial-${N_TRIAL}_training-${FOLD_INDEX}.txt 
  --valid-index  ${K_FOLD_CONFIG_ROOT}/${N_FOLD}/id-list_trial-${N_TRIAL}_test-${FOLD_INDEX}.txt 
...
cmd="${TEST_CMD} 
  --gpu ${GPU_ID} 
  --test-index ${K_FOLD_CONFIG_ROOT}/${N_FOLD}/id-list_trial-${N_TRIAL}_test-${FOLD_INDEX}.txt 
  --dataset-dir ${DATASET_DIR} 
